...
import com.basic.backend.push.api.WechatTools;
import com.basic.backend.push.core.Core;
import com.basic.backend.push.core.MsgCenter;
import com.basic.backend.push.model.vo.BaseMsg;
import com.basic.backend.push.service.ILoginService;
import com.basic.backend.push.support.enums.ResultEnum;
import com.basic.backend.push.support.enums.RetCodeEnum;
import com.basic.backend.push.support.enums.StorageLoginInfoEnum;
import com.basic.backend.push.support.enums.URLEnum;
import com.basic.backend.push.support.enums.parameters.BaseParaEnum;
import com.basic.backend.push.support.enums.parameters.LoginParaEnum;
import com.basic.backend.push.support.enums.parameters.StatusNotifyParaEnum;
import com.basic.backend.push.support.enums.parameters.UUIDParaEnum;
import com.basic.backend.push.support.util.CommonTools;
import com.basic.backend.push.support.util.Config;
import com.basic.backend.push.support.util.DateUtils;
import com.basic.backend.push.support.util.MyHttpClient;
import com.basic.backend.push.support.util.SleepUtils;
import com.basic.backend.push.thread.CheckLoginStatusThread;
...
public class LoginServiceImpl implements ILoginService {
	...
	@Override
	public void login(AsyncContext asyncContext) {
		if (core.isAlive()) { // 已登陆
			try {
				...
				responseWriter.write("机器人已有账号登录,请退出再登录");
				asyncContext.complete();
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
		String qrPath = "\\tmp\\qr\\testQr\\" + DateUtils.getCompactCurrentTime();
		while (true) {
	...
	@Override
	public boolean getQR(String qrPath, AsyncContext asyncContext) {
	...
	public boolean webWxInit() {
		...
		try {
			...
			this.core.getGroupIdList().clear();
	...
	public void startReceiving() {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				while (core.isAlive()) {
					try {
						LOG.info("startReceiving time:{}", new Date());
